...
- **Public URLs do not change.** `/districts/{districtSlug}/areas/{areaSlug}/listings` from [Session A](/docs/roadmap/notes/SESSION-A-DECISION-RECORD) survives intact, because 67 discovery roots (47 prefectures + 20 designated cities) and slug-unique-within-root are preserved as a projection of the tree.
...
Roadmap note, not a decision. It proposes amendments to [Geography](/docs/architecture/geography) and [ADR-013](/docs/architecture/decisions/adr-013-geography-reference-data) and asks Product + Engineering for a call on `AMB-036`.

| Topic | Document |
| --- | --- |
| Current geography design | [Geography](/docs/architecture/geography) |
...
| Public URL contract | [Session A — Tourist UI access model and public URL architecture](/docs/roadmap/notes/SESSION-A-DECISION-RECORD) |
| Requirements | [CAT functional requirements](/docs/requirements/functional-requirements/cat) (`FR-CAT-001`–`004`, `FR-CAT-032`) |
| Decision Log | [Open Questions](/docs/ambiguities/open-questions) (`AMB-020`, `AMB-036`) |
...
| Step | Change | Notes |
| --- | --- | --- |
| 1 | Spec first | `redcab-docs/docs/specs/{issue}-geography-administrative-tree.md`, `status: approved`, per the workspace spec-first rule |
...
| `business-rules/glossary.md` | Redefine District ("a geography node presented as a top-level navigation unit") and Area ("a listable geography node"); add Subdivision, Municipality, Ward, Listable node, Discovery root, Successor geography |
| `business-rules/invariants.md` | `INV-8`: restate over subtrees ("a node with zero Published Listings in its subtree"). `INV-11`: add the successor-pointer requirement so archive-on-merger is mechanized. `OPR-10`: cascade follows `path` prefix. No change to `PRC-1` or `BKG-11` — state that explicitly |
...
| `architecture/bounded-contexts/catalog.md` | Geography module description and the new aggregate |
...
| `ambiguities/open-questions.md` | Decision Log: revision row for `AMB-036`; Priority index line. Optionally a new `AMB-041` if the C1-vs-C2 discovery-root choice needs a separate Product decision |
| `red-cab-api/docs/db/catalog.dbml` | Authoritative schema per §3.4 |
| `roadmap/notes/SESSION-A-DECISION-RECORD.md` | Cross-reference note: URL family unaffected; the slug-resolution item in §5 is now delivered by the geography issue |
