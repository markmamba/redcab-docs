...
- **Geography** is admin-managed **reference data** (District/Area with EN/JA labels). Its only behavior — hide-if-no-published-listings (`INV-8`) and deactivate-cascades-to-listings (`OPR-10`) — is *about listings*. It has no independent ubiquitous language or lifecycle, and its rules are meaningless without Catalog. A separate context would add an interface and mappers for pure CRUD reference data — speculative generality.
