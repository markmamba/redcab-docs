...
- Canonical public path family is **`/districts/{districtSlug}/areas/{areaSlug}/listings/{listingUuid}`** per [Geography](/docs/architecture/patterns/geography). `/discover` is rejected outright.
...
| Topic | Document |
| --- | --- |
| Track plan | [Tourist UI — Pre–Phase 2](/docs/product/planning/roadmap/tourist-ui-pre-phase-2) |
| Ambiguity register | [Open Questions](/docs/product/planning/open-questions) (`AMB-022`) |
| URL hierarchy source | [Geography — Discovery and search](/docs/architecture/patterns/geography) |
...
| --- | --- | --- | --- |
| P0 | `/docs/product/planning/open-questions` | `AMB-022`; Priority index; Decision Log | Mark RESOLVED (Option A, Product Owner, 2026-09-20); move from P1 list to RESOLVED list; add Decision Log row citing this record |
...
1. **Spec `redcab-docs/docs/engineering/specs/56-tourist-access-model-and-route-contract.md`** from `template.md`, `status: approved` only after `review-implementation-spec`, with governing docs `FR-IAM-012`, `NFR-SEC-005`, `FR-CAT-003/004/005/007`, `AMB-020`, `AMB-022`, `PRC-1`, `INV-8`, and [Geography](/docs/architecture/patterns/geography).
