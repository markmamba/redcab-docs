...
The final force is **long-term vendor independence**. External payment-rail detail is isolated at the edge — capture timing (`AMB-039`), refund-failure and post-settlement dispute representation (`AMB-006`, `AMB-008`), provider selection and custody/release semantics (`AMB-040`), and the choice, MVP inclusion, and gating of the optional SMS channel (`AMB-034`) — while custody and charge topology are re-resolved as sub-merchant settlement with deferred release under [ADR-015](/docs/architecture/decisions/adr-015-payment-custody-and-control-separation) (provisional, pending counsel; supersedes Decision Log `AMB-001`, `AMB-002`, `AMB-032`). That posture is itself the evidence that the architecture accommodates either resolution *without moving a single boundary* (per [../overview.md](/docs/architecture/overview), Open Architectural Decisions). Because business truth is authored inside the contexts and external systems are reached only as capabilities behind an edge, a rail or channel can be reconfigured, or replaced, and the domain model, its invariants, and its published contracts remain untouched. The domain depends on *the capability* — funds move, a message is delivered — not on *the particular supplier of it*, and that is exactly what keeps the platform free to change suppliers over time.
...
Rejected because it forfeits long-term vendor independence for no architectural gain. The architecture isolates vendor detail at the edge (`AMB-006`, `AMB-008`, `AMB-039`, `AMB-040`, `AMB-034`; custody baseline [ADR-015](/docs/architecture/decisions/adr-015-payment-custody-and-control-separation)) and depends on *capabilities* behind published contracts and an edge, not on a particular supplier, precisely so a rail or channel can be reconfigured or replaced without moving a boundary or touching an invariant (per [../overview.md](/docs/architecture/overview)). Coupling the model to one vendor would turn every future supplier change into a domain change — the opposite of the isolation this decision preserves.
