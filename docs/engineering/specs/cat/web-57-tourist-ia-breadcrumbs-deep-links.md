...
- `app/routes/marketplace/catalog-district/marketplace-catalog-district-page-layout.jsx` — remove single-child auto-skip (`#57` follow-on); `<Breadcrumbs />` mount is in [`web-58`](/docs/engineering/specs/platform/web-58-tourist-unified-layout-shell) shell
...
- Layout shell (`#58`) — nav items, mobile pattern; **`<Breadcrumbs />` mount in shell** (this spec owns `handle.breadcrumb` data only)
...
- [ ] **Remove `CatalogGeographyNav`** from marketplace catalog layout once breadcrumbs render (avoid duplicate hierarchy; [`web-58`](/docs/engineering/specs/platform/web-58-tourist-unified-layout-shell) decision #16)
