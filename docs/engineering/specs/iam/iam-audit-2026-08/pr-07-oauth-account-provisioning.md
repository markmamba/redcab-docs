---
title: "One account provisioning path for password and OAuth signup"
sidebar_label: 7 · OAuth provisioning
sidebar_position: 8
...
**Depends on:** [PR-02](/docs/engineering/specs/iam/iam-audit-2026-08/pr-02-iam-security-fixes) — the OAuth linking gate must land first; this PR moves code around it and would make that fix harder to review if the order were reversed.
...
It stays a Service by the conventions' definition: it receives models and primitives (never a Request), owns no transaction, raises no `Errors::*`, and carries no actor prefix. `DUPLICATE_EMAIL_MESSAGE` lands here as the shared constant [PR-06](/docs/engineering/specs/iam/iam-audit-2026-08/pr-06-session-surface-symmetry) asks for.
