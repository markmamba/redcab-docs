---
title: "Thread-safe session cookies, one code path per principal"
sidebar_label: 3 · Session cookies
sidebar_position: 4
...
**Depends on:** [PR-02](/docs/engineering/specs/iam/iam-audit-2026-08/pr-02-iam-security-fixes) — overlapping edits to the authenticated controllers.
...
This is the one new concept the PR introduces, and it is the reason the audit scores Simplicity 4/5 rather than 5/5. It pays for itself by removing the duplication in § 3 and by giving [PR-02](/docs/engineering/specs/iam/iam-audit-2026-08/pr-02-iam-security-fixes)'s `RevokeAllService` a home for the namespace prefix it currently duplicates.
...
If the call-site churn makes review harder, keep the six old names as one-line delegators in the same PR and delete them in [PR-08](/docs/engineering/specs/iam/iam-audit-2026-08/pr-08-deprecations). The audit's preference is to change them now — there are only eight call sites and they are all in controllers this PR already touches.
...
Out of scope here — handled in [PR-05](/docs/engineering/specs/iam/iam-audit-2026-08/pr-05-actor-base-controllers).
...
1. Ship after [PR-02](/docs/engineering/specs/iam/iam-audit-2026-08/pr-02-iam-security-fixes), never bundled with it. PR-02 changes behaviour deliberately; this PR must change nothing observable. Mixing them makes an incident impossible to attribute.
