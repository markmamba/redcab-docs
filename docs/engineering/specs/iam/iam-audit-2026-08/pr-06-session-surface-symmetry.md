---
title: "Session surface symmetry and contract normalization"
sidebar_label: 6 · Session symmetry
sidebar_position: 7
...
- **Phase 1. Additive** — the old admin route stays until [PR-08](/docs/engineering/specs/iam/iam-audit-2026-08/pr-08-deprecations).

**Fixes:** IAM-17 (Medium), IAM-19 (Medium), IAM-23 (Medium), IAM-30 (Low).

**Depends on:** [PR-04](/docs/engineering/specs/iam/iam-audit-2026-08/pr-04-account-current-patch) — establishes `current` on the principal resource as the pattern this generalizes.
...
Byte-for-byte the same body as `SessionsController#show`. The serializer is renamed in [PR-08](/docs/engineering/specs/iam/iam-audit-2026-08/pr-08-deprecations); leave it alone here so this PR stays a pure move.
...
3. **[PR-08](/docs/engineering/specs/iam/iam-audit-2026-08/pr-08-deprecations)** deletes the branch and the nested-params test.
...
1. `team-sessions-api.js` → send flat params instead of `{ session: formData }`. Required before [PR-08](/docs/engineering/specs/iam/iam-audit-2026-08/pr-08-deprecations).
